/* ==================== renderer.ts ====================
 * 渲染层主文件（TypeScript 迁移版）。
 * 原 renderer.js 通过 <script> 加载，函数与变量为全局；
 * 现改为 ES module，用 import/export 替代全局变量。
 * 功能与原 renderer.js 完全一致，仅做类型化与模块化。
 * =============================================== */

/* ==================== 模块导入 ==================== */
// 公共工具函数从 lib 模块引入（替代原 window.RendererUtils / 全局变量）
import { genId, previewText, escapeHtml } from './lib/shared-utils.js';
import { pad2, formatChatTime, debounce } from './lib/rendererUtils.js';
import { marked } from 'marked';
import DOMPurify from 'dompurify';
// tabs 模块（ES module 版本，替代原 window.XxxModule 全局对象）
import * as CalendarTab from './tabs/calendarTab.js';
import * as MusicTab from './tabs/musicTab.js';
import * as RadioTab from './tabs/radioTab.js';

// 兼容层：保持原 window.XxxModule 接口，改为直接引用 ES module 导出
...
function getDomElement<T extends Element = HTMLElement>(id: string): T {
    return document.getElementById(id) as unknown as T;
}
const $ = getDomElement;
const noteInput = $<HTMLTextAreaElement>('noteInput');
const noteList = $<HTMLElement>('noteList');
const todoList = $<HTMLElement>('todoList');
const todoInput = $<HTMLInputElement>('todoInput');
const todoCount = $<HTMLElement>('todoCount');
const todoAddBtn = $<HTMLElement>('todoAddBtn');
const newNoteBtn = $<HTMLElement>('newNoteBtn');
const pinBtn = $<HTMLElement>('pinBtn');
const minBtn = $<HTMLElement>('minBtn');
const closeBtn = $<HTMLElement>('closeBtn');
const fixedBtn = $<HTMLElement>('fixedBtn');
const settingsAiBtn = $<HTMLElement>('settingsAiBtn');  // 复用变量名，指向设置面板内的 AI 设置按钮
const aiModal = $<HTMLElement>('aiModal');
const modalCloseBtn = $<HTMLElement>('modalCloseBtn');
const aiBaseUrl = $<HTMLInputElement>('aiBaseUrl');
const aiApiKey = $<HTMLInputElement>('aiApiKey');
const aiModelInput = $<HTMLSelectElement>('aiModelSelect');
const modelList = $<HTMLElement>('modelList');
const searchModelsBtn = $<HTMLButtonElement>('searchModelsBtn');
const modelStatus = $<HTMLElement>('modelStatus');
const aiTemperature = $<HTMLInputElement>('aiTemperature');
const tempValue = $<HTMLElement>('tempValue');
const aiPrompt = $<HTMLTextAreaElement>('aiPrompt');
const modalSaveBtn = $<HTMLButtonElement>('modalSaveBtn');
// AI 聊天模块
const chatList = $<HTMLElement>('chatList');
const chatMessages = $<HTMLElement>('chatMessages');
...
const chatNewBtn = $<HTMLElement>('chatNewBtn');
const chatDelBtn = $<HTMLElement>('chatDelBtn');
const chatTitle = $<HTMLElement>('chatTitle');
const startupToggle = $<HTMLInputElement>('startupToggle');
let startupToggleState: { open: boolean } = { open: false };

// 软件锁相关元素
const lockBtn = $<HTMLElement>('lockBtn');
const lockOverlay = $<HTMLElement>('lockOverlay');
const lockPinInputs = $<HTMLElement>('lockPinInputs');
const lockMessage = $<HTMLElement>('lockMessage');
...
const paletteGrid = $<HTMLElement>('paletteGrid');
const winEl = $<HTMLElement>('window');
const resizeHandle = $<HTMLElement>('resizeHandle');
const createPrompt = $<HTMLTextAreaElement>('createPrompt');
const createOptimizeBtn = $<HTMLButtonElement>('createOptimizeBtn');
const createGenBtn = $<HTMLButtonElement>('createGenBtn');
const createUploadBtn = $<HTMLButtonElement>('createUploadBtn');
const createFileInput = $<HTMLInputElement>('createFileInput');
const createImage = $<HTMLImageElement>('createImage');
const createPlaceholder = $<HTMLElement>('createPlaceholder');
const uploadPreview = $<HTMLElement>('uploadPreview');
const uploadPreviewImg = $<HTMLImageElement>('uploadPreviewImg');
const createCopyBtn = $<HTMLButtonElement>('createCopyBtn');
const imgProvider = $<HTMLSelectElement>('imgProvider');
const imgBaseUrl = $<HTMLInputElement>('imgBaseUrl');
const imgApiKey = $<HTMLInputElement>('imgApiKey');
const imgModel = $<HTMLInputElement>('imgModel');
const imgSaveBtn = $<HTMLButtonElement>('imgSaveBtn');
const createSize = $<HTMLSelectElement>('createSize');
const uploadFilename = $<HTMLElement>('uploadFilename');
const uploadClose = $<HTMLElement>('uploadClose');

/* ==================== 创作：图片/视频模式切换 DOM ==================== */
const createModeImageBtn = $<HTMLElement>('createModeImageBtn');
const createModeVideoBtn = $<HTMLElement>('createModeVideoBtn');
const createVideo = $<HTMLVideoElement>('createVideo');
const videoT2vModel = $<HTMLInputElement>('videoT2vModel');
const videoI2vModel = $<HTMLInputElement>('videoI2vModel');
const videoBaseUrl = $<HTMLInputElement>('videoBaseUrl');
const videoApiKey = $<HTMLInputElement>('videoApiKey');
const createModelSelect = $<HTMLSelectElement>('createModelSelect');
...
/* ==================== Markdown 预览 DOM ==================== */
const mdToggleBtn = $<HTMLElement>('mdToggleBtn');
const notePreviewWrap = $<HTMLElement>('notePreviewWrap');
const editorWrap = document.querySelector<HTMLElement>('.editor-wrap');

/* ==================== 闹钟页 DOM ==================== */
const clockDate = $<HTMLElement>('clockDate');
const clockTime = $<HTMLElement>('clockTime');
const clockWeek = $<HTMLElement>('clockWeek');
const timerDisplay = $<HTMLElement>('timerDisplay');
const timerRingFg = $<SVGPathElement>('timerRingFg');
const timerH = $<HTMLInputElement>('timerH');
const timerM = $<HTMLInputElement>('timerM');
const timerS = $<HTMLInputElement>('timerS');
const timerStartBtn = $<HTMLButtonElement>('timerStartBtn');
const timerPauseBtn = $<HTMLButtonElement>('timerPauseBtn');
const timerResetBtn = $<HTMLButtonElement>('timerResetBtn');
const alarmH = $<HTMLInputElement>('alarmH');
const alarmM = $<HTMLInputElement>('alarmM');
const alarmS = $<HTMLInputElement>('alarmS');
const alarmAddBtn = $<HTMLButtonElement>('alarmAddBtn');
const alarmList = $<HTMLElement>('alarmList');
const alarmCount = $<HTMLElement>('alarmCount');
const alarmStopBtn = $<HTMLButtonElement>('alarmStopBtn');
const timerLabel = $<HTMLInputElement>('timerLabel');
const timerSound = $<HTMLSelectElement>('timerSound');
const alarmLabel = $<HTMLInputElement>('alarmLabel');
const alarmSound = $<HTMLSelectElement>('alarmSound');

// 闹钟音量 / 开机启动弹出面板 DOM（原 renderer.js 未声明，此处补齐以避免隐式全局）
const settingsAlarmVolumeBtn = $<HTMLElement>('settingsAlarmVolumeBtn');
const alarmVolumePanel = $<HTMLElement>('alarmVolumePanel');
const alarmVolumeSlider = $<HTMLInputElement>('alarmVolumeSlider');
const alarmVolumeValue = $<HTMLElement>('alarmVolumeValue');
const settingsStartupBtn: any = $('settingsStartupBtn');
const startupPanel: any = $('startupPanel');
...
/* ==================== 状态 ==================== */
let notes: any[] = [];
let archivedNotes: any[] = [];   // 归档便签
let trashedNotes: any[] = [];    // 垃圾桶便签
let todos: any[] = [];           // 独立待办事项主数据
let archivedTodos: any[] = [];   // 归档待办事项
let currentNoteId: number | null = null;
let saveTimer: ReturnType<typeof setTimeout> | null = null;
let todoSaveTimer: ReturnType<typeof setTimeout> | null = null;
let pushTimer: ReturnType<typeof setTimeout> | null = null;          // pushNoteToPopout 的独立防抖计时器（200ms），与 saveTimer 解耦
...
// genId() 与 previewText() 已从 shared-utils.js 引入（见文件顶部 import）

/**
 * 将时间戳格式化为"年月日 时:分"
 * @param ts - 时间戳（毫秒）
 * @returns 格式如"2026年07月17日 14:30"
 */
function formatNoteCreatedTime(ts: number): string {
    if (!ts) return '';
    const d = new Date(ts);
    return `${d.getFullYear()}年${pad2(d.getMonth() + 1)}月${pad2(d.getDate())}日 ${pad2(d.getHours())}:${pad2(d.getMinutes())}`;
}

const sortNotes = (arr: any[]): any[] => arr.sort((a: any, b: any) => (b.updatedAt || 0) - (a.updatedAt || 0));

// escapeHtml 已从 shared-utils.js 引入（见文件顶部 import），本行 re-export 为死代码，已移除

// debounce / pad2 / formatChatTime 已从 rendererUtils.js 引入（见文件顶部 import）
...
function renderFolderList(opts: any): void {
    let listEl = opts.listEl;
    let countEl = opts.countEl;
    let items = opts.items;
    let keyword = opts.keyword;
    let emptyText = opts.emptyText;
    let searchEmptyText = opts.searchEmptyText;
    let titleFn = opts.titleFn;
    let metaFn = opts.metaFn;
    let searchTextFn = opts.searchTextFn;
    let restoreAction = opts.restoreAction;
    let deleteAction = opts.deleteAction;
    let restoreTitle = opts.restoreTitle;
    let infoAction = opts.infoAction;
    let sortFn = opts.sortFn;
    ...
    let filtered = kw ? items.filter(function(it: any) { return searchTextFn(it).toLowerCase().indexOf(kw) !== -1; }) : items;
...
function bindFolderSearch(inputEl: any, setKeyword: (val: string) => void, renderFn: () => void, wait?: number): void {
    if (wait === undefined) wait = 200;
    let debounced = debounce(function(val: string) {
        setKeyword(val);
        renderFn();
    }, wait);
    inputEl.addEventListener('input', function(e: any) { debounced(e.target.value); });
    inputEl.addEventListener('keydown', function(e: any) { e.stopPropagation(); });
}
...
function bindModalClose(modal: any, closeBtn: any, onClose?: () => void): void {
    let close = function() {
...
function flashBtn(btn: any, color: string, ms: number = 1500): void {
...
function setupRendererLogging(): void {
    ...
    let pending: any[] = [];                   // 待上报日志队列
    let flushTimer: ReturnType<typeof setTimeout> | null = null;              // 定时上报器
    ...
    // 入队：格式化消息 + 加入待上报队列
    function enqueue(level: string, args: any[]): void {
        try {
    ...
    console.log = function (...a: any[]) { origLog(...a); enqueue('INFO', a); };
    console.warn = function (...a: any[]) { origWarn(...a); enqueue('WARN', a); };
    console.error = function (...a: any[]) { origErr(...a); enqueue('ERROR', a); };
...
function flushSaveData(): void {
    ...
    try { window.api.saveTodos(todos); } catch (e: any) { console.error('退出时保存待办失败:', e.message); }
}
...
window.addEventListener('beforeunload', () => {
    ...
    if (pushTimer) {
        ...
        if (_note && _note.id) {
            try { window.api.pushNoteToPopout(String(_note.id), _note.content || ''); } catch (e: any) { console.warn('推送便签到小窗失败:', e.message); }
        }
    }
    // 2+3. 便签与待办刷盘
    flushSaveData();
...
const saveArchivedTodosToDisk = async (): Promise<void> => {
...
const saveSettingsToDisk = async (): Promise<void> => {
 ...
 * @param size - 字体大小（px）
 */
function applyDetailFontSize(size: number): void {
    detailFontSize = size;
    document.documentElement.style.setProperty('--detail-font-size', size + 'px');
}

/* ==================== 当前便签工具 ==================== */
const getCurrentNote = (): any => notes.find(n => n.id === currentNoteId) || null;
...
// 事件委托：点击便签列表项
noteList.addEventListener('click', (e: any) => {
    // 点击归档按钮
...
// 右键菜单：便签删除
const noteCtxMenu: any = $('noteCtxMenu');
let ctxMenuNoteId: number | null = null;
noteList.addEventListener('contextmenu', (e: any) => {
    const item = e.target.closest('.note-item');
...
// 点击菜单项
noteCtxMenu.addEventListener('click', (e: any) => {
...
// 清理 pushTimer 并补发最后一次推送，避免旧便签的延迟推送破坏新便签状态
function flushPopoutPush(): void {
    if (!pushTimer) return;
    clearTimeout(pushTimer);
    pushTimer = null;
    const oldNote = getCurrentNote();
    if (oldNote && oldNote.id && typeof window.api.pushNoteToPopout === 'function') {
        try { window.api.pushNoteToPopout(String(oldNote.id), oldNote.content || ''); } catch (e: any) { console.warn('推送便签到小窗失败:', e.message); }
    }
}
...
async function switchNote(id: number): Promise<void> {
...
function renderTodoList(): void {
    ...
    const sorted = todos.slice().sort((a: any, b: any) => {
        const aDone = !!a.done;
        const bDone = !!b.done;
...
async function saveContentAndFlush(): Promise<void> {
    if (saveTimer) { clearTimeout(saveTimer); saveTimer = null; }
...
function handleContentInput(): void {
    ...
    // 来自小窗口的更新：读取 suppressPushToPopout 后立即复位，本次不回推，避免 IPC 回环
    const skipPush = suppressPushToPopout;
    suppressPushToPopout = false;
    if (note && note.id && typeof window.api.pushNoteToPopout === 'function' && !skipPush) {
        const nid = String(note.id);
        const ncontent = note.content || '';
        if (pushTimer) clearTimeout(pushTimer);
        pushTimer = setTimeout(() => {
            try { window.api.pushNoteToPopout(nid, ncontent); } catch (e: any) { console.warn('推送便签到小窗失败:', e.message); }
            pushTimer = null;
        }, 200);
    }
    ...
    saveTimer = setTimeout(async () => {
        ...
        if (note && note.id) {
            try { await window.api.saveNoteSnapshot(note.id, note.content || ''); } catch (e: any) { console.error('保存便签历史快照失败:', e.message); }
        }
    }, SAVE_DELAY);
}
 ...
 * ============================================================ */
const noteHistoryBtn: any = $('noteHistoryBtn');
const noteHistoryPanel: any = $('noteHistoryPanel');
const noteHistoryList: any = $('noteHistoryList');

noteHistoryBtn.addEventListener('click', async (e: any) => {
    e.stopPropagation();
    const note = getCurrentNote();
 ...
 * 点击列表项：恢复内容
 */
function renderHistoryList(note: any, history: any[]): void {
    noteHistoryList.innerHTML = '';
    // 分两组：未锁定 + 锁定
    const unlocked = history.filter(h => !h.locked);
    const locked = history.filter(h => h.locked);
    // 未锁定的倒序（最新在最上）
    const sortedUnlocked = unlocked.slice().sort((a: any, b: any) => b.ts - a.ts);
    // 锁定的也倒序
    const sortedLocked = locked.slice().sort((a: any, b: any) => b.ts - a.ts);

    const appendItem = (item: any, isLocked: boolean) => {
        ...
        // 锁定按钮（图钉图标）
        const historyLockBtn = document.createElement('button');
        historyLockBtn.className = 'history-lock-btn' + (isLocked ? ' active' : '');
        historyLockBtn.title = isLocked ? '已锁死（点击解锁）' : '锁死此版本（永久保留）';
        historyLockBtn.innerHTML = isLocked
        ...
        historyLockBtn.addEventListener('click', async (ev: any) => {
            ev.stopPropagation();
            try {
            ...
            } catch (e: any) { console.warn('切换历史记录锁失败:', e.message); }
        });
        mainCol.appendChild(historyLockBtn);
...
function closeNoteHistoryPanel(): void {
 ...
 * ============================================================ */
const exportImageBtn: any = $('exportImageBtn');
exportImageBtn.addEventListener('click', async (e: any) => {
    e.stopPropagation();
    const note = getCurrentNote();
 ...
 */
function noteLineHeight(fs: number): number {
    return Math.ceil(fs * 1.75);
}

function wrapNoteText(ctx: any, text: string, maxWidth: number, font: string): string[] {
    ctx.font = font;
    const lines: string[] = [];
...
function buildNoteRenderItems(mctx: any, title: string, blocks: any[], opts: any): any[] {
...
function measureNoteHeight(renderItems: any[], padding: number): number {
...
function drawNoteToCanvas(ctx: any, renderItems: any[], opts: any): void {
...
async function renderNoteToImageDataUrl(note: any): Promise<string | null> {
    try {
        ...
        const measureCanvas: any = document.createElement('canvas');
 ...
 * @returns 块列表
 */
function parseContentToBlocks(text: string): any[] {
    if (!text) return [];
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if ((m = line.match(/^\d+\. (.+)$/)))  { const numMatch = line.match(/^\d+\./); blocks.push({ type: 'list', text: stripInline((numMatch ? numMatch[0] : '') + ' ' + m[1]) }); continue; }
        // 空行
 ...
 * ============================================================ */
const popoutNoteBtn: any = $('popoutNoteBtn');
// 记录每条便签是否已扯出小窗口（避免重复打开）
let poppedOutNoteId: string | null = null;
...
popoutNoteBtn.addEventListener('click', async (e: any) => {
    e.stopPropagation();
    const note = getCurrentNote();
    if (!note || !note.id) return;
    try {
...
if (window.api?.onPopoutNoteUpdate) {
    window.api.onPopoutNoteUpdate((data: any) => {
...
// 小窗口关闭 → 清理标记
if (window.api?.onPopoutNoteClose) {
    window.api.onPopoutNoteClose((data: any) => {
        if (!data || !data.noteId) return;
 ...
 * - 顶部含置顶切换按钮
 * ============================================================ */
const popoutTodoBtn: any = $('popoutTodoBtn');
let poppedOutTodoNoteId: string | null = null;

if (popoutTodoBtn) {
    popoutTodoBtn.addEventListener('click', async (e: any) => {
        e.stopPropagation();
        try {
            const title = '待办事项';
...
if (window.api?.onPopoutTodoUpdate) {
    window.api.onPopoutTodoUpdate((data: any) => {
        if (!data) return;
...
// 待办小窗口关闭 → 清理标记
if (window.api?.onPopoutTodoClose) {
    window.api.onPopoutTodoClose((data: any) => {
        if (!data) return;
...
aiTemperature.addEventListener('input', function(this: any) { tempValue.textContent = parseFloat(this.value).toFixed(1); });

/* ==================== 应用日志查看器（升级：级别+来源筛选 + 统计信息）==================== */
const logModal: any = $('logModal');
const logCloseBtn: any = $('logCloseBtn');
const logRefreshBtn: any = $('logRefreshBtn');
const logCopyBtn: any = $('logCopyBtn');
const logClearBtn: any = $('logClearBtn');
const logContent: any = $('logContent');
const logInfo: any = $('logInfo');
const viewLogsBtn: any = $('viewLogsBtn');
// 筛选下拉框引用
const logLevelFilter: any = $('logLevelFilter');
const logSourceFilter: any = $('logSourceFilter');

// 构建当前筛选条件对象（空字符串表示不筛该项）
function buildLogFilter(): any {
    const filter: any = {};
...
function renderLogText(text: string, prefix: string): void {
    if (text && text.trim()) {
...
async function loadLogsIntoView(): Promise<void> {
...
let logFilterDebounce: ReturnType<typeof setTimeout> | null = null;
...
const IMG_PROVIDER_PROFILES: any = {
    openai: {
...
function applyImageConfigToForm(imgCfg: any): void {
...
function applyProviderPlaceholders(provider: string): void {
...
imgProvider.addEventListener('change', async () => {
    ...
    if (oldProvider !== newProvider) {
        ...
        if (curB || curK || curM || curPath) {
            ...
            } catch (e: any) { console.warn('保存图像配置失败:', e.message); }
        }
    }
...
async function loadChats(): Promise<void> {
    try {
        const loaded = await window.api.loadChats();
        aiChats = Array.isArray(loaded) ? loaded : [];
    } catch (e: any) {
...
async function loadArchivedAndTrashedChats(): Promise<void> {
...
async function saveChats(): Promise<void> {
    try {
        await window.api.saveChats(aiChats);
    } catch (e: any) {
        console.error('保存聊天记录失败:', e);
    }
}

// formatChatTime 已从 rendererUtils.js 引入（见文件顶部 import）

function getCurrentChat(): any {
    return aiChats.find(c => c.id === currentChatId) || null;
}

function genChatTitle(text: string): string {
...
function renderChatList(): void {
    ...
    const sorted = [...aiChats].sort((a: any, b: any) => (b.updatedAt || 0) - (a.updatedAt || 0));
    const frag = document.createDocumentFragment();
    sorted.forEach(chat => {
        const item = document.createElement('div');
...
function createMessageDom(msg: any): HTMLElement {
...
function buildStatsDom(stats: any, contentForCopy: string): HTMLElement {
    ...
    copyBtn.addEventListener('click', async (ev: any) => {
        ev.stopPropagation();
        ...
        } catch (e: any) { console.warn('复制聊天内容失败:', e.message); }
    });
...
function renderMessages(): void {
    ...
    const frag = document.createDocumentFragment();
    chat.messages.forEach((msg: any) => frag.appendChild(createMessageDom(msg)));
...
function appendMessageDom(msg: any): void {
...
// 通用：从 aiChats 移除会话并处理当前会话切换（删除/归档共用）
function removeChat(id: number, target: any[]): void {
    const idx = aiChats.findIndex(c => c.id === id);
    if (idx < 0) return;
    const moved = aiChats[idx];
    target.push(moved);
...
function deleteChat(id: number): void {
    ...
    const idx = aiChats.findIndex(c => c.id === id);
    if (idx < 0) return;
    aiChats[idx].trashedAt = Date.now();
    removeChat(id, trashedChats);
    window.api.saveTrashedChats(trashedChats);
}

// 归档会话：从 aiChats 移到 archivedChats（与便签归档逻辑一致）
function archiveChat(id: number): void {
    const idx = aiChats.findIndex(c => c.id === id);
    if (idx < 0) return;
    aiChats[idx].archivedAt = Date.now();
    removeChat(id, archivedChats);
    ...
    chat.messages.forEach((m: any) => {
        if (m.role === 'user' && Array.isArray(m.images)) {
            m.images.forEach((img: any) => {
                if (img && img.path) {
                    try { window.api.deleteChatImage(img.path); } catch (e: any) { console.warn('删除聊天图片失败:', e.message); }
                }
            });
        }
    });
...
async function migrateChatImagesToDisk(): Promise<void> {
...
function renderAttachments(): void {
    ...
    chatAttachmentsList.forEach((att: any, idx: number) => {
        const item = document.createElement('div');
...
async function streamAssistantReply(chat: any): Promise<void> {
    chatSending = true;
    ...
    const isScrolledNearBottom = (el: any, threshold: number = 40): boolean => {
        if (!el) return true;
...
chatSendBtn.addEventListener('click', () => {
    // 暂停模式：点击中断当前 AI 回答
    if (chatSending) {
        try { window.api.abortChat(); } catch (e: any) { console.warn('中断 AI 回答失败:', e.message); }
        return;
    }
    sendChat();
});
 ...
 * @param isStreaming - true=AI正在回答，显示暂停按钮；false=恢复发送按钮
 */
function setSendButtonState(isStreaming: boolean): void {
    chatSendBtn.disabled = false;  // 保持可点击（正常发送 / 点击中断）
    if (isStreaming) {
...
// 思考模式开关
chatThinkingBtn.addEventListener('click', (e: any) => {
    e.preventDefault();
    e.stopPropagation();
...
chatUploadBtn.addEventListener('mousedown', (e: any) => { e.stopPropagation(); });
...
// 初始化聊天模块
async function initChatModule(): Promise<void> {
    await loadChats();
    // 加载归档/垃圾桶数据（await 确保打开文件夹时数据已就绪，防空列表；catch 防错误传播）
    try { await loadArchivedAndTrashedChats(); } catch (e: any) { console.error('加载归档/垃圾桶聊天失败:', e.message); }
    if (aiChats.length === 0) {
        newChat();
    } else {
        currentChatId = aiChats[aiChats.length - 1].id;
...
createFileInput.addEventListener('change', function(this: any) {
...
createSize.addEventListener('input', () => {
    ...
    sizeSaveTimer = setTimeout(async () => {
        ...
        try { await window.api.saveCustomSize(sz); } catch (e: any) { console.error('保存自定义尺寸失败:', e.message); }
    }, 600);
});

createSize.addEventListener('change', () => {
    const sz = createSize.value.trim();
    try { window.api.saveCustomSize(sz); } catch (e: any) { console.error('保存自定义尺寸失败:', e.message); }
});

/* ==================== 创作：图片/视频模式切换 ==================== */
function setCreateMode(mode: string): void {
    createMode = mode;
    if (mode === 'video') {
...
function updateModelSelectLabels(imgCfg: any): void {
...
/* ==================== 面板定位工具 ==================== */
function positionPanel(btn: any, panel: any): void {
    const titlebarRect = (document.querySelector('.titlebar') as HTMLElement).getBoundingClientRect();
...
/* ==================== 软件设置弹出面板（三级菜单） ==================== */
const settingsBtn: any = $('settingsBtn');
const settingsPanel: any = $('settingsPanel');
 ...
 * ============================================================ */
settingsAlarmVolumeBtn.addEventListener('click', (e: any) => {
    e.stopPropagation();
    closeAllPanels();
...
alarmVolumeSlider.addEventListener('input', function(this: any) {
...
// 松开滑块时持久化
alarmVolumeSlider.addEventListener('change', async function(this: any) {
    ...
    } catch (e: any) { console.warn('试听闹钟铃声失败:', e.message); }
});

/* ==================== 开机启动弹出面板（仿透明度面板风格） ====================
 * Toggle 开关，开启后开机自动启动到托盘
 * ============================================================ */
function updateStartupToggleUI(): void {
    if (startupToggle) startupToggle.checked = !!(startupToggleState && startupToggleState.open);
}

settingsStartupBtn.addEventListener('click', async (e: any) => {
    e.stopPropagation();
    closeAllPanels();
    // 同步一次系统状态
    try {
        ...
        updateStartupToggleUI();
    } catch (e: any) { console.warn('读取开机启动状态失败:', e.message); }
    startupPanel.classList.add('open');
    positionPanel(settingsBtn, startupPanel);
});

startupToggle.addEventListener('change', async function(this: any) {
...
/**
 * 读取一组 4 位 PIN 输入框的值
 * @param container - 包含 4 个 .lock-pin-box 的容器
 * @returns 4 位 PIN 字符串（不足 4 位返回空串）
 */
function readPinInputs(container: any): string {
    if (!container) return '';
    const boxes = container.querySelectorAll('.lock-pin-box');
    let pin = '';
    boxes.forEach((b: any) => { pin += (b.value || ''); });
 ...
 * @param container
 */
function clearPinInputsLocal(container: any): void {
    if (!container) return;
    container.querySelectorAll('.lock-pin-box').forEach((b: any) => { b.value = ''; });
 ...
 * @param container
 */
function bindPinInputAutoJump(container: any): void {
    if (!container) return;
    const boxes = container.querySelectorAll('.lock-pin-box');
    boxes.forEach((box: any, idx: number) => {
        ...
        box.addEventListener('paste', (e: any) => {
            ...
            boxes.forEach((b: any, i: number) => { b.value = digits[i] || ''; });
            if (digits.length === 4) {
...
function lockApp(): void {
    isLocked = true;
    window.api.setAppLocked(true);
    clearPinInputsLocal(lockPinInputs);
    lockMessage.textContent = '';
    lockOverlay.style.display = 'flex';
    setTimeout(() => {
        const first = lockPinInputs.querySelector<HTMLElement>('.lock-pin-box');
        if (first) first.focus();
    }, 50);
}
...
function unlockApp(): void {
    isLocked = false;
    window.api.setAppLocked(false);
    lockOverlay.style.display = 'none';
    clearPinInputsLocal(lockPinInputs);
}

// 锁按钮点击事件
lockBtn.addEventListener('click', async (e: any) => {
    e.stopPropagation();
    let hasPin = false;
...
/* ==================== 调色板弹出面板 ==================== */
settingsPaletteBtn.addEventListener('click', (e: any) => {
    e.stopPropagation();
    closeAllPanels();
    renderPalette();
...
// 事件委托：调色板点击
paletteGrid.addEventListener('click', (e: any) => {
 ...
 * 应用背景色：同时切换深色/浅色主题
 * @param hex - 颜色 hex
 * @param isDark - 是否为深色主题（需反转文字颜色）
 */
function applyColor(hex: string, isDark: boolean): void {
    selectedColor = hex;
    if (hex) {
 ...
 * ============================================================ */
const settingsFontBtn: any = $('settingsFontBtn');
const fontSetPanel: any = $('fontSetPanel');
const fontSizeSlider: any = $('fontSizeSlider');
const fontSizeValue: any = $('fontSizeValue');
const fontSizeResetBtn: any = $('fontSizeResetBtn');
const DEFAULT_DETAIL_FONT_SIZE = 14;

settingsFontBtn.addEventListener('click', (e: any) => {
    e.stopPropagation();
    closeAllPanels();
...
const fmSettingsPanel: any = $('fmSettingsPanel');
...
async function saveFmConfigFromForm(): Promise<boolean> {
...
function showFmError(msg: string): void {
...
if (settingsFmBtn) {
    settingsFmBtn.addEventListener('click', async (e: any) => {
        e.stopPropagation();
        closeAllPanels();
...
let fmSaveTimer: ReturnType<typeof setTimeout> | null = null;
function fmScheduleSave(): void {
    if (fmSaveTimer) clearTimeout(fmSaveTimer);
...
/* ==================== 数据同步设置（S3 / WebDAV） ==================== */
const settingsSyncBtn: any = $('settingsSyncBtn');
const syncModal: any = $('syncModal');
const syncCloseBtn: any = $('syncCloseBtn');
const syncStatus: any = $('syncStatus');
// S3 字段
const s3Endpoint: any = $('s3Endpoint');
const s3Region: any = $('s3Region');
const s3Bucket: any = $('s3Bucket');
const s3AccessKey: any = $('s3AccessKey');
const s3SecretKey: any = $('s3SecretKey');
const s3Path: any = $('s3Path');
const s3SaveBtn: any = $('s3SaveBtn');
const s3TestBtn: any = $('s3TestBtn');
const s3SyncBtn: any = $('s3SyncBtn');
const s3RestoreBtn: any = $('s3RestoreBtn');
// WebDAV 字段
const webdavUrl: any = $('webdavUrl');
const webdavUser: any = $('webdavUser');
...
const webdavRestoreBtn: any = $('webdavRestoreBtn');
// 自动同步
const autoSyncToggle: any = $('autoSyncToggle');
const autoSyncInterval: any = $('autoSyncInterval');
// 恢复备份模态框
const backupModal: any = $('backupModal');
const backupCloseBtn: any = $('backupCloseBtn');
const backupList: any = $('backupList');
const backupModalTitle: any = $('backupModalTitle');

let currentSyncTab = 's3';

// 设置同步状态提示
function setSyncStatus(msg: string, type?: string): void {
    syncStatus.textContent = msg;
    syncStatus.className = 'sync-status' + (type ? ' ' + type : '');
}

// 获取当前 S3 配置（从输入框读取）
function getS3ConfigFromInputs(): any {
    return {
...
function getWebdavConfigFromInputs(): any {
    return {
...
// 加载同步配置到输入框
async function loadSyncConfigToUI(): Promise<void> {
    try {
        ...
        // 激活对应的 tab
        document.querySelectorAll('.sync-tab').forEach((b: any) => {
            b.classList.toggle('active', b.dataset.syncTab === currentSyncTab);
        });
        document.querySelectorAll('.sync-tab-content').forEach((c: any) => {
...
// 打开同步模态框
settingsSyncBtn.addEventListener('click', (e: any) => {
    e.stopPropagation();
    closeAllPanels();
...
// Tab 切换
document.querySelectorAll('.sync-tab').forEach((btn: any) => {
    btn.addEventListener('click', () => {
        currentSyncTab = btn.dataset.syncTab;
        document.querySelectorAll('.sync-tab').forEach((b: any) => b.classList.remove('active'));
        document.querySelectorAll('.sync-tab-content').forEach((c: any) => c.classList.remove('active'));
        btn.classList.add('active');
        (document.querySelector('[data-sync-content="' + currentSyncTab + '"]') as HTMLElement).classList.add('active');
    });
});
...
// 接收自动同步结果
if (window.api?.onAutoSyncResult) {
    window.api.onAutoSyncResult((data: any) => {
...
// 打开恢复备份模态框并加载列表
async function openBackupModal(provider: string): Promise<void> {
...
// 渲染备份列表
function renderBackupList(items: any[]): void {
...
// 恢复单个备份
async function restoreBackupItem(item: any): Promise<void> {
...
// 删除单个备份
async function deleteBackupItem(item: any): Promise<void> {
 ...
 * ============================================================ */
const archiveFolderBtn: any = $('archiveFolderBtn');
const trashFolderBtn: any = $('trashFolderBtn');
const archiveModal: any = $('archiveModal');
const archiveCloseBtn: any = $('archiveCloseBtn');
const archiveList: any = $('archiveList');
const archiveCount: any = $('archiveCount');
const trashModal: any = $('trashModal');
const trashCloseBtn: any = $('trashCloseBtn');
const trashList: any = $('trashList');
const trashCount: any = $('trashCount');
const trashClearBtn: any = $('trashClearBtn');
...
function renderArchiveList(): void {
    renderFolderList({
        ...
        titleFn: function(n: any) { return previewText(n.content); },
        metaFn: function(n: any) {
        ...
        searchTextFn: function(n: any) { return n.content || ''; },
...
const todoArchiveFolderBtn: any = $('todoArchiveFolderBtn');
const todoArchiveModal: any = $('todoArchiveModal');
const todoArchiveCloseBtn: any = $('todoArchiveCloseBtn');
const todoArchiveList: any = $('todoArchiveList');
const todoArchiveCount: any = $('todoArchiveCount');
const todoArchiveSearchInput: any = $('todoArchiveSearchInput');

/**
 * 渲染待办归档文件夹列表（支持搜索关键字过滤）
 */
function renderTodoArchiveList(): void {
    renderFolderList({
...
if (todoArchiveFolderBtn) {
    todoArchiveFolderBtn.addEventListener('click', (e: any) => {
        e.stopPropagation();
...
if (todoArchiveSearchInput) {
    bindFolderSearch(todoArchiveSearchInput, function(val: string) { todoArchiveSearchKeyword = val; }, renderTodoArchiveList);
}
...
// 待办归档列表事件委托：还原 / 永久删除
if (todoArchiveList) {
    todoArchiveList.addEventListener('click', async (e: any) => {
...
// 工具栏：打开归档文件夹
if (archiveFolderBtn) {
    archiveFolderBtn.addEventListener('click', (e: any) => {
        e.stopPropagation();
        archiveSearchKeyword = '';
        const archiveSearchEl = $<HTMLInputElement>('archiveSearchInput');
        if (archiveSearchEl) archiveSearchEl.value = '';
        renderArchiveList();
...
// 工具栏：打开垃圾桶
trashFolderBtn.addEventListener('click', (e: any) => {
    e.stopPropagation();
    // 打开时清空搜索框，显示全部
    trashSearchKeyword = '';
    const trashSearchEl = $<HTMLInputElement>('trashSearchInput');
    if (trashSearchEl) trashSearchEl.value = '';
    renderTrashList();
 ...
 * ============================================================ */
// 便签列表搜索
const noteSearchInput: any = $('noteSearchInput');
// 防抖搜索：用户连续输入时只在停顿 200ms 后渲染一次，避免大列表每次按键都全量重建
bindFolderSearch(noteSearchInput, function(val: string) { noteSearchKeyword = val; }, renderNoteList);

// 归档文件夹搜索
const archiveSearchInput: any = $('archiveSearchInput');
bindFolderSearch(archiveSearchInput, function(val: string) { archiveSearchKeyword = val; }, renderArchiveList);

// 垃圾桶搜索
const trashSearchInput: any = $('trashSearchInput');
bindFolderSearch(trashSearchInput, function(val: string) { trashSearchKeyword = val; }, renderTrashList);
...
// 归档列表事件委托：还原 / 永久删除
archiveList.addEventListener('click', async (e: any) => {
    const actionEl = e.target.closest('[data-action]');
    if (!actionEl) return;
    const item = e.target.closest('.folder-item');
    if (!item || !item.dataset.id) return;
...
// 垃圾桶列表事件委托：还原 / 永久删除
trashList.addEventListener('click', async (e: any) => {
    const actionEl = e.target.closest('[data-action]');
    if (!actionEl) return;
    const item = e.target.closest('.folder-item');
    if (!item || !item.dataset.id) return;
...
// 垃圾桶：清空所有便签
trashClearBtn.addEventListener('click', async (e: any) => {
    e.stopPropagation();
 ...
 * ============================================================ */
const chatArchiveFolderBtn: any = $('chatArchiveFolderBtn');
const chatTrashFolderBtn: any = $('chatTrashFolderBtn');
const chatArchiveModal: any = $('chatArchiveModal');
const chatArchiveCloseBtn: any = $('chatArchiveCloseBtn');
const chatArchiveListEl: any = $('chatArchiveList');
const chatArchiveCountEl: any = $('chatArchiveCount');
const chatTrashModal: any = $('chatTrashModal');
const chatTrashCloseBtn: any = $('chatTrashCloseBtn');
const chatTrashListEl: any = $('chatTrashList');
const chatTrashCountEl: any = $('chatTrashCount');
const chatTrashClearBtn: any = $('chatTrashClearBtn');

// 提取聊天正文用于搜索与预览（拼接所有消息文本）
function getChatSearchText(chat: any): string {
    if (!chat) return '';
    const title = chat.title || '';
    const msgs = (chat.messages || []).map((m: any) => m.content || '').join(' ');
    return (title + ' ' + msgs);
}

function previewChatTitle(chat: any): string {
 ...
 * 渲染聊天归档列表（支持搜索关键字过滤）
 */
function renderChatArchiveList(): void {
    renderFolderList({
        ...
        titleFn: function(c: any) { return previewChatTitle(c); },
        metaFn: function(c: any) {
 ...
 * 渲染聊天垃圾桶列表（支持搜索关键字过滤）
 */
function renderChatTrashList(): void {
    renderFolderList({
...
// 工具栏：打开聊天归档文件夹
chatArchiveFolderBtn.addEventListener('click', (e: any) => {
    e.stopPropagation();
    chatArchiveSearchKeyword = '';
    const inp = $<HTMLInputElement>('chatArchiveSearchInput');
    if (inp) inp.value = '';
...
// 工具栏：打开聊天垃圾桶
chatTrashFolderBtn.addEventListener('click', (e: any) => {
    e.stopPropagation();
    chatTrashSearchKeyword = '';
    const inp = $<HTMLInputElement>('chatTrashSearchInput');
    if (inp) inp.value = '';
    renderChatTrashList();
 ...
 * ============================================================ */
const chatExportMdBtn: any = $('chatExportMdBtn');
chatExportMdBtn.addEventListener('click', async (e: any) => {
    e.stopPropagation();
    const chat = getCurrentChat();
 ...
 * @returns
 */
function buildChatMarkdown(chat: any): string {
    const lines: string[] = [];
    ...
    (chat.messages || []).forEach((msg: any, idx: number) => {
        ...
        if (msg.ts) {
            try {
                ...
                lines.push('');
            } catch (e: any) { console.warn('格式化消息时间失败:', e.message); }
        }
        // 思考过程
        if (msg.role === 'assistant' && msg.reasoning) {
            lines.push('<details>');
        ...
        if (msg.role === 'user' && Array.isArray(msg.images) && msg.images.length > 0) {
            ...
            msg.images.forEach((img: any, i: number) => {
...
// 聊天归档搜索
const chatArchiveSearchInput: any = $('chatArchiveSearchInput');
// 搜索框防抖 200ms（与其余搜索框一致，降低高频输入时的 DOM 重建开销）
bindFolderSearch(chatArchiveSearchInput, function(val: string) { chatArchiveSearchKeyword = val; }, renderChatArchiveList);

// 聊天垃圾桶搜索
const chatTrashSearchInput: any = $('chatTrashSearchInput');
bindFolderSearch(chatTrashSearchInput, function(val: string) { chatTrashSearchKeyword = val; }, renderChatTrashList);
...
// 聊天归档列表事件委托：还原 / 永久删除
chatArchiveListEl.addEventListener('click', async (e: any) => {
    const actionEl = e.target.closest('[data-action]');
    if (!actionEl) return;
    const item = e.target.closest('.folder-item');
    if (!item || !item.dataset.id) return;
...
// 聊天垃圾桶列表事件委托：还原 / 永久删除
chatTrashListEl.addEventListener('click', async (e: any) => {
    const actionEl = e.target.closest('[data-action]');
    if (!actionEl) return;
    const item = e.target.closest('.folder-item');
    if (!item || !item.dataset.id) return;
...
// 聊天垃圾桶：清空所有聊天
chatTrashClearBtn.addEventListener('click', async (e: any) => {
    e.stopPropagation();
...
if (resizeHandle) {
    ...
    // 键盘方向键调整窗口大小（无障碍支持）
    resizeHandle.addEventListener('keydown', (e: any) => {
...
(function setupModalFocusTrap() {
    ...
    let modalStack: any[] = [];
    function activeModal(): any { return modalStack.length > 0 ? modalStack[modalStack.length - 1] : null; }
    function isActive(modal: any) { return modalStack[modalStack.length - 1] === modal; }

    // 获取模态框内所有可聚焦元素（过滤不可见、禁用、或祖先隐藏的）
    function getFocusableElements(modal: any): any[] {
        const selector = 'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])';
        return Array.from(modal.querySelectorAll(selector)).filter(function(el: any) {
    ...
    function onModalClose(modal: any) {
        var idx = modalStack.indexOf(modal);
        ...
        if (modalStack.length === 0) {
            ...
            if (lastFocusedElement && typeof lastFocusedElement.focus === 'function') {
                ...
                setTimeout(function() {
                    try { restoreTarget.focus(); } catch (e: any) { console.warn('恢复焦点失败:', e.message); }
                }, 50);
            }
        } else {
            // 还有外层模态，聚焦到外层模态的关闭按钮
    ...
    modals.forEach(function(modal: any) {
...
// 内容编辑区可在便签页和闹钟页间共享移动
const contentPanel: any = $('contentPanel');
const notesContentSlot: any = $('notesContentSlot');
const clockContentSlot: any = $('clockContentSlot');

function moveContentPanelTo(slot: any): void {
    if (slot && contentPanel.parentElement !== slot) {
        ...
        try { renderEditor(); } catch (e: any) { console.warn('重新渲染编辑器失败:', e.message); }
    }
}
...
/* ==================== Markdown 预览切换 ====================
 * 使用 npm 包 marked 与 DOMPurify 进行 Markdown 转换与 HTML 净化
 */
...
// 便签切换时若处于预览模式，刷新预览
function refreshMarkdownPreviewIfActive(): void {
    if (editorWrap && editorWrap.classList.contains('preview-mode')) {
        renderMarkdownPreview();
    }
}
...
// pad2 已抽取至 rendererUtils.js（见文件顶部引用）
...
let masterGainNode: any = null;   // 主增益节点：实现音量放大（>100% 时强行增益）
let alarmRinging = false;
let alarmOscTimer: ReturnType<typeof setTimeout> | null = null;
...
function ensureAudioCtx(): any {
    if (!audioCtx) {
        ...
        catch (e: any) { console.error('AudioContext 创建失败', e); }
    }
 ...
 * @param percent - 音量百分比
 */
function setAlarmVolume(percent: number): void {
    const v = Math.max(0, Math.min(300, parseInt(String(percent), 10) || 0));
    alarmVolume = v;
...
// 基础音符播放（ADSR 包络）
function playNote(ctx: any, freq: number, startTime: number, duration: number, type?: string, vol?: number): void {
...
function startAlarmSound(soundType?: string): void {
    ...
    } catch (e: any) { console.warn('恢复音频上下文失败:', e.message); }
    alarmRinging = true;
...
function startTimer(): void {
    ...
    [timerH, timerM, timerS].forEach((i: any) => i.disabled = true);
}
...
function stopTimer(): void {
    ...
    [timerH, timerM, timerS].forEach((i: any) => i.disabled = false);
}
...
function onTimerFinished(): void {
    ...
    } catch (e: any) { console.warn('发送倒计时通知失败:', e.message); }
    ...
    if (document.hidden && window.api && typeof window.api.showWindowForAlarm === 'function') {
        try { window.api.showWindowForAlarm(); } catch (e: any) { console.warn('唤起主窗口失败:', e.message); }
    }
}
...
/* ==================== 定时闹钟模块 ==================== */
let alarms: any[] = []; // { id, h, m, s, enabled, triggered }
...
function saveAlarms(): void {
    try { localStorage.setItem('alarms', JSON.stringify(alarms)); } catch (e: any) { console.error('保存闹钟失败:', e.message); }
}
...
function toggleAlarm(id: number): void {
...
function renderAlarmList(): void {
    alarmCount.textContent = String(alarms.length);
    if (alarms.length === 0) {
    ...
    alarms.forEach(a => {
        ...
        // 铃声名称映射
        const soundNames: any = { default: '默认叮咚', apple: '苹果风格', android: '安卓风格', nokia: '诺基亚经典', crystal: '清脆铃声', bird: '鸟鸣', electronic: '电子闹钟' };
...
// 事件委托
alarmList.addEventListener('click', (e: any) => {
...
/* ==================== 闹钟/倒计时 tab 切换 ==================== */
document.querySelectorAll('.alarm-tab').forEach((tab: any) => {
    tab.addEventListener('click', function(this: any) {
        const target = this.dataset.alarmTab;
        document.querySelectorAll('.alarm-tab').forEach((t: any) => t.classList.remove('active'));
        this.classList.add('active');
        document.querySelectorAll('.alarm-panel').forEach((p: any) => p.classList.remove('active'));
...
function checkAlarms(): void {
    ...
    if (anyRinging) {
        ...
        } catch (e: any) { console.warn('发送闹钟通知失败:', e.message); }
...
function renderAlarmListInfoOnly(): void {
 ...
 * ================================================================== */
async function loadAllAiConfigsToForm(): Promise<void> {
    // 1. AI 聊天配置
    try {
...
async function initAppSettings(): Promise<void> {
    ...
    } catch (e: any) { console.warn('读取自定义尺寸失败:', e.message); }
}

async function initWindowProps(): Promise<void> {
    try {
        const pinned = await window.api.getPinState();
        pinBtn.classList.toggle('active', pinned);
        pinBtn.title = pinned ? '取消置顶' : '窗口置顶';
    } catch (e: any) { console.warn('读取置顶状态失败:', e.message); }
    try {
        const f = await window.api.getFixedState();
        fixedBtn.classList.toggle('active', f);
        fixedBtn.title = f ? '已固定（点击解除）' : '固定窗口位置';
    } catch (e: any) { console.warn('读取固定状态失败:', e.message); }
    if (window.api?.onFixedChanged) {
        window.api.onFixedChanged((f: boolean) => {
            fixedBtn.classList.toggle('active', f);
            fixedBtn.title = f ? '已固定（点击解除）' : '固定窗口位置';
        });
    }
    if (window.api?.onPinChanged) {
        window.api.onPinChanged((pinned: boolean) => {
            pinBtn.classList.toggle('active', pinned);
            pinBtn.title = pinned ? '取消置顶' : '窗口置顶';
        });
    }
...
function initChatAndAi(): void {
    initChatModule();
    migrateChatImagesToDisk().catch((e: any) => console.warn('图片迁移任务异常:', e.message));
    loadAllAiConfigsToForm();
}

function initAlarmsAndClock(): void {
...
(async function init() {
    try {
        // 兼容层：把 calendarTab.ts 依赖的全局变量/函数挂载到 window
        // calendarTab.ts 通过 `declare const` 声明这些变量，运行时从全局查找
        // 使用 getter 确保 notes/alarms 重新赋值后 window 上的引用同步更新
        Object.defineProperty(window, 'notes', { get: () => notes, configurable: true });
        Object.defineProperty(window, 'alarms', { get: () => alarms, configurable: true });
        (window as any).saveAlarms = saveAlarms;
        (window as any).saveNotesToDisk = saveNotesToDisk;
        (window as any).renderNoteList = renderNoteList;
        (window as any).switchNote = switchNote;
        ...
        initChatAndAi();
        initAlarmsAndClock();
        // 初始化日历模块（原 calendar.js 在 DOMContentLoaded 时自动 init，
        // 现改为 ES module 后由 renderer.ts 显式调用）
        CalendarTab.initCalendar();
    } catch (err: any) {
