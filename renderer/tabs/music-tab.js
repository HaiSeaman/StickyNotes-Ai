...
(function () {
    ...
    // ============ 状态 ============
    let playlist = [];           // [{filePath,title,artist,album,duration,coverPath,size,addedAt}]
    let currentIndex = -1;       // 当前播放索引（-1 表示无）
    let isPlaying = false;
    let playMode = 'sequential'; // 'sequential' | 'shuffle' | 'single'
    let volume = 80;             // 0-100
    let howl = null;             // Howl 实例
    let searchQuery = '';
    let shuffleHistory = [];     // 随机模式下的历史索引栈（用于"上一首"回溯）
    let dragSrcIdx = -1;         // 拖拽排序：源索引（模块级，避免每次 render 重建闭包）
    let rafId = null;            // requestAnimationFrame ID（用于取消，避免 rAF 循环累积）
    let loadErrorCount = 0;      // 连续加载失败计数（防止全部损坏时 next 无限循环）
    let isSeeking = false;       // 用户正在拖动进度条（拖动期间暂停写入进度，避免回跳）
    let inited = false;          // P6 懒初始化标志：首次进入音乐 tab 时才执行 init()
    let showFavoritesOnly = false; // 收藏夹过滤开关：true 时仅显示 track.favorite === true 的曲目
    let lastVolumeBeforeMute = 80; // 静音前的音量记忆（用于恢复）
    ...
    const savePlaylistDebounced = debounce(() => {
        if (window.api && window.api.musicSavePlaylist) {
            window.api.musicSavePlaylist(playlist).catch(e => console.warn('[Music] 保存播放列表失败:', e));
        }
    }, 1500);
    // 搜索防抖（200ms，沿用项目约定）
    ...
    function buildCoverUrl(coverPath) {
        if (!coverPath) return '';
    ...
    function playTrack(index, manual) {
        ...
        howl.on('play', () => {
            isPlaying = true;
            updatePlayButton();
            // 取消上一个 rAF 循环（避免 end→play 切换时累积多个循环）
        ...
        howl.on('end', () => {
            ...
            if (playMode === 'single') {
                ...
                setTimeout(() => next(true), 0);  // 延迟到下一 tick，避免在 end 事件中同步卸载当前 Howl
            }
        });
        howl.on('loaderror', () => {
    ...
    function deleteTrack(index) {
        ...
        // 删除前如果正在播放该曲，先卸载 Howl 并取消 rAF，避免切换期间残留
        if (wasCurrent) {
    ...
    function updateProgress() {
        if (isSeeking) { rafId = requestAnimationFrame(updateProgress); return; }
        if (!howl || !isPlaying) { rafId = null; return; }
        const dur = howl.duration() || 0;
    ...
    window.MusicModule = {
        ...
        onTabActivated: function () {
            // P6 懒初始化：首次进入音乐 tab 时才执行 init()，避免启动期 IPC 与 DOM 渲染拖慢首屏
            if (!inited) {
                inited = true;
    ...
    // P6 懒初始化：不再在 DOMContentLoaded 时自动 init()，改由 onTabActivated 首次触发
})();
