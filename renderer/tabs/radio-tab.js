...
(function () {
    ...
    // ============ 状态 ============
    let stations = [];          // 当前显示的电台列表 [{name,url,favicon,country,tags,bitrate,codec,homepage}]
    let topStations = [];       // 热门电台缓存（避免每次切 tab 都请求）
    let favorites = [];         // 收藏电台列表
    let currentIndex = -1;      // 当前播放索引（-1 表示无）
    let currentStation = null;  // 当前播放的电台对象（用于收藏判断 / UI 显示）
    let isPlaying = false;
    let currentTab = 'hot';     // 'hot' | 'fav' | 'search'
    let audio = null;           // HTML5 Audio 实例（单例，init 中创建）
    let playTimeoutId = null;   // 10s 播放超时计时器（未进入 playing 视为失败）
    let autoNextTimeoutId = null; // 2s 自动切换计时器（流错误后延迟切换）
    let consecutiveErrors = 0;  // 连续错误计数（用于"全部失败"判定）
    let streamErrorHandled = false;  // 同一电台的错误处理守卫（避免 error 事件与超时/拒绝重复触发）
    let inited = false;             // P6 懒加载标志：首次进入 radio tab 时才执行 init()
    let searchQuery = '';       // 上次搜索关键词（切回 search tab 时复用）
    let config = {};            // 持久化配置 { lastTab, lastStationUrl }
    let refreshSourceIndex = 0;     // 刷新按钮当前数据源索引（0-4 循环切换 5 种来源）
    let isRefreshing = false;       // 刷新按钮重入守卫（避免短时间内重复点击触发并发刷新）
    const REFRESH_SOURCES = ['topvote', 'topclick', 'recent', 'bycountry-china', 'bycountry-hongkong'];
    let netSpeedInterval = null;    // 网络速度采样计时器（每秒更新一次）
    let lastBufferedEnd = 0;        // 上次采样的 buffered.end 值（秒）
    let lastSampleTime = 0;         // 上次采样的时间戳（ms）
    let showFavoritesOnly = false;  // 收藏夹筛选开关：true 时仅显示已收藏的电台
    let isCtrlRefreshing = false;   // 控制栏刷新按钮重入守卫
    let cachedDataSource = '';      // topStations 缓存对应的数据源（用于检测数据源切换后清空旧缓存）
    ...
    const saveFavoritesDebounced = debounce(() => {
        if (window.api && window.api.radioSaveFavorites) {
            window.api.radioSaveFavorites(favorites).catch(e => { console.warn('[Radio] 保存收藏失败:', e); reportLog('warn', ['[Radio] 保存收藏失败:', e]); });
        }
    }, 1500);
    // 配置持久化防抖（1.5s）
    const saveConfigDebounced = debounce(() => {
        if (window.api && window.api.radioSaveConfig) {
            window.api.radioSaveConfig(config).catch(e => { console.warn('[Radio] 保存配置失败:', e); reportLog('warn', ['[Radio] 保存配置失败:', e]); });
        }
    }, 1500);
    // 搜索防抖（200ms）
    const onSearchInputDebounced = debounce((q) => {
        searchStations(q);
    }, 200);

    // ============ 工具函数 ============
    function defaultStationSvg() {
    ...
    function isBenignPlayRejection(err) {
        if (!err) return false;
    ...
    function setupAudioEvents() {
        ...
        audio.addEventListener('error', (e) => {
            console.error('[Radio] 音频错误:', e, audio.error);
            reportLog('error', ['[Radio] 音频错误:', e, audio.error]);
            stopNetSpeedMonitor();
            setStatusBarPlaying(false);
            handleStreamError();
        });
    ...
    function bindEvents() {
        ...
        if (els.tabBtns) {
            els.tabBtns.addEventListener('click', (e) => {
        ...
        // 搜索
        if (els.searchInput) {
            els.searchInput.addEventListener('input', (e) => onSearchInputDebounced(e.target.value));
        ...
        if (els.stationsContainer) {
            els.stationsContainer.addEventListener('click', (e) => {
                // 先检查是否点击了操作按钮（收藏/删除）
    ...
    async function loadCnHkMusicStations() {
        if (!isOnline()) {
            updateStatus('网络未连接');
            return;
        }
    ...
    function updateStreamInfo(station) {
    ...
    // ============ 状态栏填充动画 ============
    function setStatusBarPlaying(playing) {
    ...
    async function refreshAllStations() {
        ...
        try {
            ...
            if (currentTab !== 'hot') {
                ...
                if (els.tabBtns) {
                    const btns = els.tabBtns.querySelectorAll('[data-tab]');
                    btns.forEach(b => b.classList.toggle('active', b.dataset.tab === 'hot'));
                }
                // 隐藏搜索框
    ...
    async function refreshStationsBySource() {
    ...
    window.RadioModule = {
        init,
        // 暴露给外部调用的方法（供 renderer.js 在 tab 切换时调用）
        onTabActivated: function () {
            // P6 懒加载：首次进入 radio tab 时才执行 init()，避免启动时即创建 Audio / 拉取电台列表
            if (!inited) {
                inited = true;
