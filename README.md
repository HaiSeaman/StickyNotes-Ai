...
**StickyNotes-Ai** 是一款专为 Windows 平台打造的下一代桌面便签与效率工作台。它融合了媲美 Windows 11 的 Fluent / Acrylic 毛玻璃半透明视觉设计，支持多便签管理、独立置顶拆分、沉浸式音视频播放、S3 / WebDAV 多端云同步，并内置最新升级的 **v2.9 稳定性与性能优化**：修复 5 项关键 Bug、清理约 950 行冗余代码、搜索引擎与音乐 / 电台模块全面重构，为用户带来优雅、流畅、强大的桌面个人知识与创作助理体验。音乐播放器支持**收藏夹播放模式**，一键激活后随机/顺序播放仅限收藏曲目。

---

## 🚀 版本更新亮点

### 🛡️ v2.9 稳定性与性能全面优化 (New)

**关键 Bug 修复（5 项 P0）**：
- **修复首次启动空库无法使用**：新建用户首次打开便签时输入框与"+"新建按钮失效的严重问题，现已正常响应。
- **修复软件锁 PIN 无法修改 / 清除**：设置 PIN 后原先无修改与清除入口（用户将被永久锁死），现设置面板新增「软件锁 PIN」管理入口，支持验证当前 PIN 后修改或清除。
- **修复视频生成无法中止**：AI 生成视频点击"停止"无效的问题，生成中再次点击按钮即可立即中止任务。
- **修复视频任务轮询监听器泄漏**：长轮询循环中事件监听器逐轮累积泄漏的内存隐患。
- **修复窗口兜底定时器崩溃**：窗口提前关闭时后台定时器触发非空断言导致应用异常退出的隐患。

**性能优化**：
- 聊天图片异步读取不再阻塞主进程（20MB 大图不再卡 UI）。
- 音乐文件夹扫描并发加速（2000 个文件不再串行 realpath）。
- 备份增加 200MB 内存上限保护；自动同步自动排除大体积聊天图片目录。
- 会话列表改为事件委托渲染，大幅减少监听器与重绘开销。

**代码质量重构（约 950 行瘦身）**：
- 5 个搜索引擎 Provider 抽取公共骨架，消除 70% 复制粘贴代码。
- FM 电台加载 / 音乐曲目构建等重复逻辑统一收敛。
- 清理 6 个从未被调用的死 IPC 通道、整份死类型契约、2 个冗余 npm 依赖与多处残留文件。

**测试强化**：单元测试 64 → 80 个，新增搜索引擎工具与本地文件协议 Range 读取回归测试。
...
│   │       └── syncService.ts         # WebDAV 多端同步服务（S3 同步见 main.ts）
